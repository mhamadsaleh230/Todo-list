...
import "./othertodos.css";
import Editform from '../form/editform';

export function Othertodos({ todos = [], refreshTodayTodos, refreshOtherTodos }) {
  const [editTodo, setEditTodo] = useState(null);

  const handleCheckboxChange = (todo) => {
    axios.put('http://localhost:8081/completetodos', {
      id: todo.id,
  ...
  const deletetodo = (id) => {
    axios.delete('http://localhost:8081/deletetodo', { data: { id } })
      .then(() => {
        refreshOtherTodos();
      })
            ...
            {editTodo && editTodo.id === todo.id ? (
              <Editform
                ...
                onClose={() => {
                  setEditTodo(null);
                  refreshOtherTodos();
                  refreshTodayTodos(); // update both lists on edit
                }}
              />
            ) : (
              <div className="card">
                <table style={{ tableLayout: 'fixed', width: '100%' }}>
                  ...
                  <tbody>
                    <tr>
                      ...
                      <td data-completed={todo.iscompleted}>{todo.todo}</td>
                    </tr>
                    ...
                    <tr>
                      <td style={{ textAlign: 'right', fontWeight: 'bold', verticalAlign: 'top' }}>Date : </td>
                      <td data-completed={todo.iscompleted}>{new Date(todo.date).toLocaleDateString()}</td>
                    </tr>
                    <tr>
